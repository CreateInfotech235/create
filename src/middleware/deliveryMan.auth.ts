...
export default async (
  ...
  try {
    ...
    if (!bearerToken?.includes('Bearer')) {
      // return res.badRequest({ statusCode : 1000 , message: getLanguage('en').invalidToken });
      return res.status(1000).json({ status : 1000 , message: getLanguage('en').invalidToken });
    }
    console.log("Hello");
    ...
    if (!tokenExpired) {
      // return res.badRequest({statusCode : 1000 , message: getLanguage('en').invalidToken });
      // return res.status(1000).json({ status : 1000 , message: getLanguage('en').invalidToken1 });
    ...
    if (
      ...
      // });
      return res.status(1000).json({ status : 1000 , message: getLanguage('en').errorDocumentVerified }); 
    }
